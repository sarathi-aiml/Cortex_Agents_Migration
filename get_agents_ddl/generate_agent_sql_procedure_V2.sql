...
def generate_agent_sql(
    ...
    if "instructions" in agent_spec and agent_spec["instructions"]:
        ...
        def _append_instr_field(key: str, val: str):
            if not val:
                return
            # Use block scalar when text has newlines or inner quotes to avoid YAML breakage.
            if ("\n" in val) or ('"' in val):
                yaml_content.append(f"  {key}: |")
                for line in val.splitlines():
                    yaml_content.append(f"    {line}")
            else:
                yaml_content.append(f'  {key}: "{val}"')

        _append_instr_field("response", instructions.get("response", ""))
        _append_instr_field("orchestration", instructions.get("orchestration", ""))
        _append_instr_field("system", instructions.get("system", ""))
